...
class WebpackConfig {
     ...
     * Uses babel.loadPartialConfigAsync() and caches the result.
     *
    ...
    configureBabel(callback, options = {}) {
        if (callback) {
            ...
            // Note: the check for external Babel config is deferred to build
            // time (in config-generator.js) because it requires an async call.
        }
        ...
        // The actual check for external Babel config is deferred to
        // build time (in config-generator.js) because it requires
        // an async call.
