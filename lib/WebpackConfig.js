...
class WebpackConfig {
    ...
    enableImagemin(options = {}) {
        ...
        for (const optionKey of Object.keys(options)) {
            if (!(optionKey in this.imageminOptions)) {
                throw new Error(`Invalid option "${optionKey}" passed to enableImagemin(). Valid keys are ${Object.keys(this.imageminOptions).join(', ')}`);
            }

            this.imageminOptions[optionKey] = options[optionKey];
        }
    }
