...
Encore
    ...
    // a script tag to the built file ({outputPath}/layout.js) in
    // your base layout. You can also require CSS from this file.
    // If you do, also add a link tag to the built CSS file ({outputPath}/layout.css).
    ...
    // addEntry('otherPage', './assets/js/other.js')
    ...
    // create hashed filenames (e.g. layout.abc123.css)
    // .enableVersioning()
...
require('../css/layout.${this.initConfig.cssFileExtension}');
...
#webpack-encore {
    background: lightblue;
        ...
        mkdirp.sync(path.join(this.initConfig.projectPath, this.initConfig.assetRelativeDir, 'css'));
        mkdirp.sync(path.join(this.initConfig.projectPath, this.initConfig.assetRelativeDir, 'js'));

        return this._writeFileSafe(path.join(this.initConfig.assetRelativeDir, 'js', 'layout.js'), jsContents)
            .then(() => this._writeFileSafe(path.join(this.initConfig.assetRelativeDir, 'css', 'layout.' + this.initConfig.cssFileExtension), cssContents));
    }
...
module.exports = function(initConfig, options) {
    ...
    return generator.addDependencies(options.installDeps)
        ...
        .then(() => generator.createEntryFiles())
        ...
        .then(() => {
            ...
            console.log(`  along with new ${chalk.yellow('layout.js')} and ${chalk.yellow('layout.' + initConfig.cssFileExtension)} files to get you started.`);
            console.log('');
